...
def parse_arguments():
    ...
    parser.add_argument("read_coverage")
    ...
    sample_size = ast.literal_eval(args.sample_size)
    read_coverage = ast.literal_eval(args.read_coverage)
    n_trees = args.n_trees
    
    return repSaveData,n_clone,n_mutation,cell_size_min,cell_size_max,sample_size,read_coverage,n_trees
...
def VAF(n_sample,n_clone,cell_size,n_mutation,read_cov,root,edges):
    ...
    new_mutation_in_clone = Clone.copy()
...
def Data_MIPUP(F,n_mutation,n_sample):
    ...
    #Data_MIPUP.to_csv('Data_MIPUP.txt',sep = '\t',header = False,index = False) 
    return Data_MIPUP 
...
def Data_LICHeE(F,n_mutation,n_sample):
    ...
    #Data_LICHeE.to_csv('Data_LICHeE.txt',sep = '\t',header = False,index = False)
    return Data_LICHeE
...
def Data_AncesTree(n,x,n_mutation,n_sample):
    ...
    #Data_AncesTree.to_csv('Data_AncesTree.txt',sep = '\t',header = False,index = False)
...
def Data_Treeomics(n,x,n_mutation,n_sample):
    ...
    #cov.to_csv('Data_Treeomics_n.txt',sep = '\t',header = False,index = False)
    ...
    mu = pd.DataFrame(mu)  
    #mu.to_csv('Data_Treeomics_x.txt',sep = '\t',header = False,index = False)
    return cov,mu
...
repSaveData,n_clone,n_mutation,cell_size_min,cell_size_max,sample_size,read_coverage,n_trees = parse_arguments()
mutation = range(n_mutation); mutation = np.array(mutation)

os.mkdir(repSaveData + '/Data_MIPUP')
os.mkdir(repSaveData + '/Data_LICHeE')
os.mkdir(repSaveData + '/Data_AncesTree')
os.mkdir(repSaveData + '/Data_CITUP')
os.mkdir(repSaveData + '/Data_Treeomics')

for n_sample in sample_size:
    for read_cov in read_coverage: 
        os.mkdir(repSaveData + '/%d_%d'%(n_sample,read_cov))
        os.mkdir(repSaveData + '/Data_MIPUP/%d_%d'%(n_sample,read_cov))
        os.mkdir(repSaveData + '/Data_LICHeE/%d_%d'%(n_sample,read_cov))
        os.mkdir(repSaveData + '/Data_AncesTree/%d_%d'%(n_sample,read_cov))
        os.mkdir(repSaveData + '/Data_CITUP/%d_%d'%(n_sample,read_cov))
        os.mkdir(repSaveData + '/Data_Treeomics/%d_%d'%(n_sample,read_cov))
        
        for ind in range(1,n_trees+1):
            root,edges,cell_size = tree(n_clone,n_mutation,cell_size_min,cell_size_max)
            U,B,F,F_unpack,n,x,F_unpack_noise,samples,G_Dir,new_mutation_in_clone,Clone = VAF(n_sample,n_clone,cell_size,n_mutation,read_cov,root,edges)
                 
            
            f = open(repSaveData + '/%d_%d/%d_%d_%d.txt'%(n_sample,read_cov,n_sample,read_cov,ind),'w') 
            f.write(str(root)+ '\n'+'\r')
            f.write(str(edges)+ '\n'+'\r')
            ...
            f.flush()
            f.close()
            nx.write_gpickle(G_Dir,repSaveData + '/%d_%d/%d_%d_%d_G_Dir.gpickle'%(n_sample,read_cov,n_sample,read_cov,ind))
            ...
            data_mipup.to_csv(repSaveData + '/Data_MIPUP/%d_%d/Data_MIPUP_%d.txt'%(n_sample,read_cov,ind),sep = '\t',header = False,index = False)
            data_lichee.to_csv(repSaveData + '/Data_LICHeE/%d_%d/Data_LICHeE_%d.txt'%(n_sample,read_cov,ind),sep = '\t',header = False,index = False)
            data_ancestree.to_csv(repSaveData + '/Data_AncesTree/%d_%d/Data_AncesTree_%d.txt'%(n_sample,read_cov,ind),sep = '\t',header = False,index = False)
            header='Num_mutations: %d\nNum_samples: %d\nError_rate: %f\n'%(n_mutation,n_sample,error_rate)
            np.savetxt(repSaveData + '/Data_CITUP/%d_%d/Data_CITUP_%d.txt'%(n_sample,read_cov,ind), data_citup, delimiter=' ', newline='\n', header=header,comments='')
            cov.to_csv(repSaveData + '/Data_Treeomics/%d_%d/Data_Treeomics_n_%d.txt'%(n_sample,read_cov,ind),sep = '\t',header = False,index = False)
            mu.to_csv(repSaveData + '/Data_Treeomics/%d_%d/Data_Treeomics_x_%d.txt'%(n_sample,read_cov,ind),sep = '\t',header = False,index = False)
